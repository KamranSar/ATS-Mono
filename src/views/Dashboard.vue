<template>
  <div class="ma-0 pa-0">
    <v-row no-gutters >
        ...
        <v-btn x-large class="secondary " href="/offendertransfer">New Offender Transfer</v-btn>
    <v-btn x-large class="secondary  ml-2" href="/schedule">Go to Schedules</v-btn>
</v-col>
...
<v-data-table
      ...
      @keypress="filterOffenderTransfers"
      :loading="loading"
    ...
    >
      <template v-slot:top>
        ...
        dense
          ...
          clearable
        ></v-text-field></v-col><v-col cols="7"><v-btn small class="secondary mt-2"><v-icon left>mdi-printer</v-icon>Print All Departing 135's</v-btn></v-col>
        </v-row>
      </template>
    ...
    <template v-slot:item.print135="{ item }">
      ...
      <v-icon color="green" class="ml-3" v-if='item.updates == true'>mdi-check-bold</v-icon>
    </template>
    ...
    >
      <template v-slot:top>
        ...
        dense
          ...
          prepend-inner-icon="mdi-magnify"
          clearable
        ></v-text-field></v-col><v-col cols="7"><v-btn small class="secondary mt-2"><v-icon left>mdi-printer</v-icon>Print All Arriving 135's</v-btn></v-col>
        </v-row>
      </template>
      <template v-slot:item.fullName="{ item }">
      <span>{{item.firstName}} {{item.lastName}}</span>
      ...
      <template v-slot:item.updates="{ item }">
      <v-icon class="ml-3" v-if='item.updates == true'>mdi-check-bold</v-icon>
    </template>
    <template v-slot:item.cdcrNumber="{ item }">
      <router-link to="">{{item.cdcrNumber}}</router-link>
    </template>
    <template v-slot:item.preprint135="{ item }">
      <router-link to="">{{item.preprint135}}<v-icon color="primary" class="ml-7">mdi-file-document</v-icon></router-link>
    </template>
    </v-data-table>
    <v-row>
      ...
      </v-btn>Last SOMS Sync: <span class="primary--text font-weight-black">2021-05-05 1:22pm</span>
      </span>
      </v-col>
    </v-row>
  </div>
...
export default {
  ...
  methods: {
    ...
    filterOffenderTransfers (value, search, item) {
        return value != null &&
          search != null &&
          typeof value === 'string' &&
      ...
      convertDepartingDates() {
        this.departingOffenders.forEach((element) => {
          element.endorseDate ? (element.endorseDate = new Date(element.endorseDate).toISOString().split('T')[0]) : null;
        });
        this.departingOffenders.forEach((element) => {
          element.releaseDate ? (element.releaseDate = new Date(element.releaseDate).toISOString().split('T')[0]) : null;
        });
      },
    ...
    created() {
      DataService.getDepartingOffenders()
        .then((response) => {
          ...
          DataService.getArrivingOffenders()
            .then((respsonse) =>{

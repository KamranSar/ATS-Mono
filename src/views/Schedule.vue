...
<div>
   ...
   <h1>Schedules</h1>
   <v-data-table  :headers="headersSchedule" @click:row="rowClick" item-key="scheduleId" :items="schedules" single-select sort-by="destination" class="elevation-1"
 > 
      ...
      <v-toolbar flat color="white">
        ...
        <v-dialog v-model="dialogSchedule" max-width="800px">
          ...
          <v-card>
              ...
              <v-container>
                <v-row>
                  ...
                  <v-col cols="12" sm="6" md="4">
                    <v-text-field v-model="editedSchedule.destination" label="Destination"></v-text-field>
                  </v-col>
                  <v-col cols="12" sm="6" md="4">
                     <v-text-field v-model="editedSchedule.schedule" label="Schedule"  ></v-text-field>                                               
                  </v-col>
                  
                  <v-col cols="12" sm="6" md="4">
                    
                    <v-select v-model="editedSchedule.VIA" :items="VIAs" attach chips label="VIAs" multiple   >

                    </v-select>
                  </v-col>
                  <v-col cols="12" sm="6" md="4">
                    <v-text-field v-model="editedSchedule.transferDate" label="Transfer Date"></v-text-field>
                  </v-col>
                  <v-col cols="12" sm="6" md="4">
                    <v-text-field v-model="editedSchedule.seats" label="Seats"></v-text-field>
                  </v-col>
                  <v-col cols="12" sm="6" md="4">
                    <v-text-field v-model="editedSchedule.remainingSeats" label="Remaining Seats"></v-text-field>
                  </v-col>
                  <!-- <v-col cols="12" sm="6" md="4">
            ...
            <v-card-actions>
              <v-spacer></v-spacer>
              <v-btn color="blue darken-1" text @click="closeSchedule">Cancel</v-btn>
        ...
        <v-dialog v-model="dialogDeleteSchedule" max-width="500px">
                    <v-card>
                      ...
                      <v-card-actions>
                        <v-spacer></v-spacer>
                        <v-btn color="blue darken-1" text @click="closeDeleteSchedule">Cancel</v-btn>
                        <v-btn color="blue darken-1" text @click="deleteScheduleConfirm()">Delete Schedule</v-btn>
                        <v-spacer></v-spacer>
                      </v-card-actions>
                    </v-card>
                  </v-dialog>
    ...
    <template v-slot:item.actions="{ item }">
      ...
      <v-icon
        small
        @click="deleteSchedule(item,schedule._id)"
      >
        mdi-delete
      </v-icon>
    </template>
...
<div id="endTable" v-show="isShowing">
  <div>
      <v-toolbar >
        ...
        <v-btn  class="secondary ma-2"  @click="isShowing ^= true" >Close</v-btn> 
        </v-toolbar-items>
    </v-toolbar>
    </div>
 <!-- <v-row class="my-4" no-gutters >
  ...
  <v-data-table  :headers="headersEndInmates"  :items="endInmates"  sort-by="lastName" class="elevation-1">
    <template v-slot:top>
      <v-toolbar flat color="white">
          ...
          <v-card>
            ...
            <v-card-text>
              <v-container>
                <v-row >
                  <v-col cols="12" sm="6" md="4">
                    <v-text-field v-model="editedEndInmate.cdcrNumber" label="CDCR Number"></v-text-field>
                  </v-col>
                  <v-col cols="12" sm="6" md="4">
                    <v-text-field v-model="editedEndInmate.lastName" label="Last Name"></v-text-field>
                  </v-col>
                  <v-col cols="12" sm="6" md="4">
                    <v-text-field v-model="editedEndInmate.firstName" label="First Name"></v-text-field>
                  </v-col>
                  <v-col cols="12" sm="6" md="4">
                    <v-text-field v-model="editedEndInmate.housing" label="Housing"></v-text-field>
                  </v-col>
                  <v-col cols="12" sm="6" md="4">
                    <v-text-field v-model="editedEndInmate.transferReason" label="Transfer Reason"></v-text-field>
                  </v-col>
                  <v-col cols="12" sm="6" md="4">
                    <v-text-field v-model="editedEndInmate.endorsementDate" label="Endorsement Date"></v-text-field>
                  </v-col>
                  <v-col cols="12" sm="6" md="4">
                    <v-text-field  label="Endorsement Details"></v-text-field>
                  </v-col>
                 <!-- <template v-slot:item.print135="{ item }">
                <router-link to="">{{item.print135}}<v-icon color="primary" class="ml-5">mdi-file-document</v-icon></router-link>
              </template> -->
            ...
            <v-card-actions>
              <v-spacer></v-spacer>
              <v-btn color="blue darken-1" text @click="closeEndInmate">Cancel</v-btn>
              <v-btn color="blue darken-1" text @click="saveEndInmate">Save</v-btn>
            </v-card-actions>
          </v-card>
        </v-dialog>
        <v-dialog v-model="dialogDeleteEndInmate" max-width="500px">
          <v-card>
          <v-card-title class="headline">Are you sure you want to delete this inmate?</v-card-title>
          <v-card-actions>
          <v-spacer></v-spacer>
          <v-btn color="blue darken-1" text @click="closeDeleteEndInmate">Cancel</v-btn>
          <v-btn color="blue darken-1" text @click="deleteEndInmateConfirm()">Delete Inmate</v-btn>
          <v-spacer></v-spacer>
          </v-card-actions>
          </v-card>
          </v-dialog>
    ...
    <router-link to="">{{item.print}}<v-icon color="primary" class="ml-5">mdi-file-document</v-icon></router-link>
    </template>
    <template v-slot:no-data>
      <span>No Results</span>
    </template> 
...
/* Schedule Script Begin    */ 
  export default {
    ...
    computed: {
      formScheduleTitle () {
        return this.editedScheduleIndex === -1 ? 'New Schedule' : 'Edit Schedule'
      },
      formEndInmateTitle () {
        return this.editedEndInmateIndex === -1 ? 'New Endorsement' : 'Edit Endorsement'
      },
    },
    ...
    rowClick: function (item, row) {   
      ...
      deleteSchedule (schedule) {
        ...
        confirm('Are you sure you want to delete this schedule?') && this.schedules.splice(index, 1)
      },
      deleteEndInmate (endInmate) {
        const index = this.endInmates.indexOf(endInmate)
        confirm('Are you sure you want to delete this Inmate?') && this.endInmates.splice(index, 1)
      },
      ...
      saveSchedule () {
        if (this.editedScheduleIndex > -1) {
          Object.assign(this.schedules[this.editedScheudleIndex], this.editedSchedule)
        } else {
      ...
      saveEndInmate () {
        if (this.editedEndInmateIndex > -1) {
          Object.assign(this.endInmates[this.editedEndInmateIndex], this.editedEndInmate)
        } else {

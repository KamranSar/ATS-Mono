...
export class VuexPersistence<S> implements PersistOptions<S> {
  ...
  public constructor(options?: PersistOptions<S>) {
    ...
    this.RESTORE_MUTATION = function RESTORE_MUTATION(
      state: S,
      savedState: any
    ) {
      ...
      for (const propertyName of Object.keys(mergedState as {})) {
        (this as any)._vm.$set(
          state,
          propertyName,
          (mergedState as any)[propertyName]
        );
      }
    };
    ...
    if (this.asyncStorage) {
      ...
      this.plugin = (store: Store<S>) => {
        ...
        (store as any).restored.push(
          (this.restoreState(this.key, this.storage) as Promise<S>).then(
            (savedState) => {
              /**
              ...
              if (this.strictMode) {
                ...
                store.replaceState(
                  merge(store.state, savedState || {}, this.mergeOption) as S
                );
              }
